...
public class EmployeeStreamsTest {
    ...
    public void findAll_nameLengthAtMost5_sortBySalary() {
        List<Employee> result = allEmployees.stream()
                ...
                .sorted(Comparator.comparing(Employee::getSalary)) // Comparator.comparing(emp -> emp.getSalary)
                .collect(Collectors.toList());
    ...
    public void threeHighestPaid_sortByName() {
        List<Employee> result = allEmployees.stream()
                .sorted((emp1, emp2) -> emp2.getSalary().compareTo(emp1.getSalary())) //Comparator.comparing(Employee::getSalary).reversed()
                .limit(3)
                .sorted(Comparator.comparing(Employee::getName)) //Comparator.comparing(emp -> emp.getName())
                .collect(Collectors.toList());
        dump(result);
    ...
    public void payAll_salaryAtMost50000_lowestPaidFirst() {
         allEmployees.stream()
                ...
                .forEach(Employee::pay); // emp -> emp.pay
    }
    ...
    public void giveRaise_salaryLessThan50000_hiredAfter2000_lowestPaidFirst() {
        List<Employee> result = allEmployees.stream()
                .filter(emp -> emp.getSalary() < 50000 && emp.getHireDate().getYear() > 2000)
                .sorted(Comparator.comparing(Employee::getSalary))
    ...
    public void allEmployeesHired2000_orLater() {
        ...
        boolean hiredAfter2k = allEmployees.stream()
                .allMatch(emp -> emp.getHireDate().getYear() >= 2000);

        System.out.println(hiredAfter2k);
    }
    ...
    public void averageSalary_hired2000_orLater() {
        ...
        Double salaryAvg = allEmployees.stream()
                .filter(emp -> emp.getHireDate().getYear() >= 2000)
                .collect(Collectors.averagingDouble(Employee::getSalary)); // emp -> emp.getSalary()
